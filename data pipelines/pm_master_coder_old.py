# -*- coding: utf-8 -*-
"""
Created on Sun Jul 10 16:34:35 2022
...
##############################################################
###### READING TSP MASSES and CALCULATING THEIR ERRORS #######
##############################################################

df = pd.read_excel(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\DES\Filters_weight_gain_master.xlsx'), sheet_name = 'Gravimetric_analysis_refined')
'''
...
# (df['TSP Check'] == df['TSPm_am_c']).unique()
...
#### Calculating time error (4b)
df['abs mass change'] = ((df['m_2_c_cD'] - df['m_3']).abs())/df['m_2_c_cD']
# df['abs mass change'] = ((df['m_2_c_cD'] - df['m_3']).abs())/df['m_2_f']
df_time_error = df[['SN', 'site', 'round', 'ft', 'abs mass change']].dropna()

df_time_error = df.groupby('ft', as_index = False)['abs mass change'].agg('median')
error_time_dict = dict(zip(df_time_error['ft'], df_time_error['abs mass change']))

'''
df['test'] = df['ft'].map(error_time_dict)
df = df[['site', 'round', 'test', 'TSP mass']]

'''

df.loc[df['ft'] == 1, 'error perc'] = error_time_dict[1]
df.loc[df['ft'] == 2, 'error perc'] = error_time_dict[2]
df.loc[df['ft'] == 3, 'error perc'] = error_time_dict[3]
df.loc[df['ft'] == 4, 'error perc'] = error_time_dict[4]

#### Adding pre-deploy and 
error_device = 0.023
error_in = 0.04 # interpretted as pre-deploy filter mass error 


# df['error time'] = np.sqrt((df['m_2_f'] * df['error perc']).pow(2) + 1.3 ** 2) # This 1.3 is due to change of the mass of clean filter wil almost zero filt vol
# df['error time'] = np.sqrt((df['m_2_f'] * df['ft'].map(error_time_dict)).pow(2) + 1.3 ** 2) # This 1.3 is due to change of the mass of clean filter wil almost zero filt vol


# df['TSP mass error'] = 2*error_device**2 + 2*error_in**2

df['TSP mass error'] = np.sqrt((df['m_2_f'] * df['error perc']).pow(2) + 1.3 ** 2 + 2 * error_in ** 2 + 2 * error_device ** 2)

### Exception handlings for site 3 filters (do in a separate df)
df_s3 = df[df['site'] == 3]
df_s3['TSP mass error'] = df_s3['TSP mass error'].pow(2)
...
df_s3['TSP mass error'] = np.sqrt(df_s3['TSP mass error'])
...
# pm_master_df.sort_values('SN', inplace = True)
# pm_master_df.sort_values(['round', 'site'], inplace = True)
...
'''
Understanding STATA sources of errors for TSP mass:
    1) error_in - my guess : short-term error of 
...
##############################################################
###### READING IN FILTRATION VOLUMES AND THEIR ERRORS ########
##############################################################

### Reading correct filtration volume for site 13 round 1 (not full 3 months)

df_time = pd.read_excel(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\DES\3_month_schedule_all.xlsx'))

df_time = df_time[['date_start', 'date_end']]
# df_time = df_time.iloc[12,:]
# df_time = df_time['12'].str[0:10]
...
df_daily = pd.read_stata(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\DES\filtration volume by day.dta'))
...
df_val = pd.read_excel(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\DES\filtration_volume_values.xlsx'), header = None)
# df_val['site'] = df_val.index + 1
...
df_err = pd.read_excel(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\DES\filtration_volume_errors.xlsx'), header = None)
...
####################################################################
###### READING IN PM2.5 and PM10 Mastersizer Mass Fractions ########
####################################################################

df_sfr = pd.read_excel(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\Processed\psd\natl_psd_master.xlsx'))
Size = df_sfr['Size']
...
sn_summary = pd.read_excel(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\Processed\sn_summary.xlsx'))
...
##################################################################
###### READING IN PM2.5 and PM10 Mini-WRAS Mass Fractions ########
##################################################################

df_eff = pd.read_csv(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\DES\eff_effectiveness.csv'))
...
#######################################################################
###### Combining All DFs and Calculation of TSP, PM10, and PM25 #######
#######################################################################
...
pm_master_df.to_excel(backslash_correct(r'C:\Career\Learning\Python Practice\Stata_Python_Booster\PhD - PSD TSP\Processed\pm_master.xlsx'), index = False)
